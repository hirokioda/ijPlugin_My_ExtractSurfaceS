...
public class My_ColorConvertRGBtoBGR implements PlugInFilter {
	ImagePlus imp;
	...
	public void run (ImageProcessor ip) {   // 一番目のチャネルにdapiのsmoothingしたファイルをセットしておく
		...
		int[] col={2,1,0,3,4,5,6,7,8,9,10};   // 色変換用の配列。
		if(nCh==2){                           // 130509 2-chanel hyperstackに対応するために修正
			col[0]=0;
			col[1]=1;
			col[2]=2;
		}
		...
		ImagePlus imp_hyp;
		imp_hyp=IJ.createHyperStack("Output_ColorConverted", w, h, nCh, nSl, nFr, 8);
		if(convertCol(imp,col,imp_hyp)==false)
			{IJ.error("The colors can not be converted!"); return;}     //  impの第２引数で指定するchannelのstackを削除して他をimp_hypにコピーする。
...
// -----------------------------------------------------------------------------	
	...
	// col[]の配列に合わせて色を変換する。 by Hiroki Oda   2013.4.7
		public boolean convertCol(ImagePlus imp_h1, int[] col, ImagePlus imp_h2){
			ImageStack[] stack_h1 = getEachStack(imp_h1);
			ImageStack[] stack_h2 = getEachStack(imp_h2);
			ImageProcessor ip_h1, ip_h2;
			...
			if(ch1!=imp_h2.getNChannels())return false;
			
			for(int ns=1; ns<=imp_h1.getNSlices(); ns++){
				for(int c=0; c<ch1; c++){
					ip_h1=stack_h1[c].getProcessor(ns);
					ip_h2=stack_h2[col[c]].getProcessor(ns);
					for(int j=0; j<imp_h1.getHeight(); j++){
						for(int i=0; i<imp_h1.getWidth();i++){
							value=ip_h1.getPixel(i, j);
							ip_h2.putPixel(i, j, value);
